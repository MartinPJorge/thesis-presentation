...
\begin{frame}
    ...
    \begin{figure}
        ...
        \caption{
            \only<1>{Orchestration of three services.}
...
\begin{frame}
    ...
    \begin{minipage}{0.35\textwidth}
        This part derives \textbf{location} of:
        \begin{enumerate}
        ...
        \vspace{1em}
        \only<2>{
            for \textbf{augmented reality}:
            \begin{itemize}
                \item tactile latency 1ms
            \end{itemize}
        }
    \end{minipage}
    \begin{minipage}{0.6\textwidth}
        \begin{figure}
            \centering
            \only<1>{\includegraphics[width=\textwidth]{img/big-picture-infra-gen-highlight.pdf}}%
            \only<2>{\includegraphics[width=\textwidth]{img/big-picture-infra-gen-highlight-ar.pdf}}
...
%% ------------------------
%% OLD FIG TO INTRODUCE WHAT WE DO
%% ------------------------
%% \begin{frame}
%%     \frametitle{\secname}
%%     \framesubtitle{\subsecname}
...
\begin{frame}
    ...
    \begin{figure}
        ...
        %% \input{grid.tex}
        \caption{BS location -- inhomogeneous Mattérn~II process.}
...
\begin{frame}
    ...
    \begin{minipage}{.48\textwidth}
        ...
        \begin{itemize}
            ...
            \item satisfy 1ms tactile latency
        \end{itemize}
    \end{minipage}
    \begin{minipage}{.5\textwidth}
        \begin{figure}
...
\begin{frame}
    ...
    \centering{
        \only<1>{$m_M$: maximum distance between server at ring $M$ and BS}
        \only<2>{$m_2$: maximum distance between server at ring $2$ and BS}
    }
    \begin{figure}
        \centering
...
\begin{figure}
    ...
    \subfigure[FDD 120 kHz 7s]{ \includegraphics[clip, trim=0cm 6cm 0.25cm 3cm, width=0.38\textwidth]{img/madrid-ii-m2.png}}
        ~
    \subfigure[TDD 120 kHz 7s]{\includegraphics[clip, trim=0cm 6cm 0.25cm 3cm, width=0.38\textwidth]{img/madrid-i-iii-m2.png}}\\
    \caption{\textbf{Urban scenario} (Madrid city center) -- $C_{A,M2}=$covered BSs by server at M2.}
    \label{fig:madrid}
\end{figure}
...
\begin{figure}
    \centering
    \subfigure[FDD 120 kHz 7s]{\includegraphics[clip, trim=0cm 11cm 0.25cm 6.7cm, width=0.485\textwidth]{img/hoces-del-cabriel-ii-m1.png}}
     ~                       
    \subfigure[TDD 120 kHz 7s]{\includegraphics[clip, trim=0cm 11cm 0.25cm 6.7cm, width=0.485\textwidth]{img/hoces-del-cabriel-i-iii-m1.png}}\\
    \caption{\textbf{Highway scenario} (Hoces del Cabriel A3) -- $C_{A,M1}=$covered BSs by server at M1.}
    \label{fig:madrid}
\end{figure}
...
\begin{frame}
    ...
    \begin{figure}
        \centering
        \only<1>{\includegraphics[width=.8\textwidth]{img/big-picture-infra-gen.pdf}}%
        \only<2>{\includegraphics[width=.8\textwidth]{img/big-picture-federation.pdf}}
        \caption{\only<1>{Infrastructure design.}
            \only<2>{Local and federation operator.}
        }
        \label{fig:recap-infra-gen}
    \end{figure}
\end{frame}
...
\begin{frame}
    ...
    \only<1>{\begin{figure}
        \centering
        \includegraphics[width=.7\textwidth]{img/federation.pdf}
    ...
    \only<2>{
        \begin{minipage}{.35\textwidth}
            {\color{red}\textbf{New}} in SoA:
            \begin{itemize}
                \item dynamic pricing
                \item real-price traces AWS
                \item Deep Q-learning
                \item Telefónica scenario
            \end{itemize}
        \end{minipage}
...
\begin{frame}
    ...
    \begin{figure}
        \centering
        \only<1>{\includegraphics[width=.7\textwidth]{img/federation-deploy-2.pdf}}%
        \only<2>{\includegraphics[width=.7\textwidth]{img/federation-deploy-we-are-2.pdf}}%
        \only<3>{\includegraphics[width=.7\textwidth]{img/federation-deploy-changes-2.pdf}}%
        \only<4>{\includegraphics[width=.7\textwidth]{img/federation-deploy-they-change-2.pdf}}%
...
\begin{frame}
    ...
    \begin{minipage}{.45\textwidth}
        ...
        \definecolor{localblue}{RGB}{200,223,229}
        \begin{multline}
...
\begin{frame}
    ...
    \begin{figure}
        \centering
        \includegraphics[width=.7\columnwidth]{img/shrink-federated.eps}
        \caption{Cumulative reward vs. available federation resources.}
    \end{figure}
\end{frame}
...
\begin{frame}
    ...
    \only<1>{
        \begin{figure}
            \centering
            \includegraphics[width=.8\textwidth]{img/big-picture-vne.pdf}
            \caption{Services' embedding.}
        \end{figure}
    }
    \only<2>{
        \begin{minipage}{.3\textwidth}
            OKpi {\color{red}\textbf{new}} embedding algo. in SoA:
            \begin{itemize}
                \item latency constraints
                \item radio coverage
                \item geographical availability
                \item reliability
            \end{itemize}
        \end{minipage}
        \begin{minipage}{.65\textwidth}
            \begin{figure}
                \centering
                \includegraphics[width=\textwidth]{img/big-picture-vne.pdf}
                \caption{Services' embedding.}
            \end{figure}
        \end{minipage}
...
\begin{frame}
    ...
    \textbf{OKpi} solves the VNE problem.

    \begin{figure}
        \only<1>{\includegraphics[width=.55\textwidth]{img/fresco_var_thesis_scenario_base.pdf}}%
        \only<2>{\includegraphics[width=.55\textwidth]{img/fresco_var_thesis_scenario_cloud.pdf}}%
        \only<3>{\includegraphics[width=.55\textwidth]{img/fresco_var_thesis_scenario_edge.pdf}}%
        \only<4>{\includegraphics[width=.55\textwidth]{img/fresco_var_thesis_scenario_fog.pdf}}
...
\begin{frame}
    ...
    \begin{figure}
        \subfigure[$\gamma=0$]{\includegraphics[width=.34\textwidth]{img/expanded-gamma-0.pdf}}\hspace{1.5em}\vline\hspace{1.5em}
        \subfigure[$\gamma=3$]{\includegraphics[height=.6\textheight]{img/expanded-gamma-3.pdf}}
...
\begin{frame}
    ...
    \begin{minipage}{.39\textwidth}
        \textbf{Decision graph}
        
        \begin{itemize}
            \item edge $(\tilde{n}_1,\tilde{n}_2)$ weights:
                \begin{equation}
                    \only<1>{\left( \frac{\tilde{D}_{\tilde{n}_1,\tilde{n}_2}}{D(s)}, \frac{\log \tilde{\eta}_{\tilde{n}_1,\tilde{n}_2}}{\log H(s)} \right)}%
                    \only<2>{\Bigg( \highlight{ \frac{\tilde{D}_{\tilde{n}_1,\tilde{n}_2}}{D(s)} }{delay fraction}, \frac{\log \tilde{\eta}_{\tilde{n}_1,\tilde{n}_2}}{\log H(s)} \Bigg)}%
                    \only<3>{\Bigg( \frac{\tilde{D}_{\tilde{n}_1,\tilde{n}_2}}{D(s)}, \highlight{ \frac{\log \tilde{\eta}_{\tilde{n}_1,\tilde{n}_2}}{\log H(s)} }{reliability fraction} \Bigg)}%
                \end{equation}%
        \end{itemize}
    \end{minipage}
    \begin{minipage}{.6\textwidth}
        \begin{figure}
            \includegraphics[width=.8\textwidth]{img/decision-graph-edges-inter.pdf}%
    ...
    \begin{minipage}{.39\textwidth}
        ...
        \begin{enumerate}
            \item add superscripts $\tilde{n}^{d,r}$
                ...
                \end{itemize}
            \onslide<2->{\item do $(\gamma+1)^2$ replicas}
            \onslide<3->{\item paths of up to $2\cdot\gamma$ hops}
            \onslide<4->{\item embed across any path}
        \end{enumerate}
    \end{minipage}
    \begin{minipage}{.6\textwidth}
        \begin{figure}
            ...
            \only<3>{\includegraphics[width=.8\textwidth]{img/expanded-feasible-connections.pdf}}%
            \only<4>{\includegraphics[width=.8\textwidth]{img/expanded-solution-1.pdf}}%
            \only<5>{\includegraphics[width=.8\textwidth]{img/expanded-solution-2.pdf}}%
...
\begin{frame}
    ...
    \only<1>{
        \begin{figure}
            \includegraphics[width=.8\textwidth]{img/big-picture-scaling.pdf}
            \caption{V2N service scaling.}
        \end{figure}
    }
    \only<2>{
        \begin{minipage}{.3\textwidth}
            {\color{red}\textbf{New}} V2N scaling using:
            \begin{itemize}
                \item time-series forecasting
                \item preemptive scaling based on forecast
            \end{itemize}
        \end{minipage}
        \begin{minipage}{.65\textwidth}
            \begin{figure}
                \includegraphics[width=\textwidth]{img/big-picture-scaling.pdf}
                \caption{V2N service scaling.}
            \end{figure}
        \end{minipage}
    }
...
\begin{frame}
    ...
    \only<1>{
        \begin{figure}
            \includegraphics[width=.8\textwidth]{img/v2n-scaling-same-style.pdf}
    ...
    \only<2>{
        ...
        \begin{minipage}{0.6\textwidth}
            \begin{figure}
                \centering
                \includegraphics[width=\textwidth]{img/v2n-scaling-same-style.pdf}
                \caption{V2N service vertical scaling.}
            \end{figure}
        \end{minipage}
    }
...
\newcounter{backupfigcounter}
\setcounter{backupcounter}{\value{framenumber}}
\setcounter{backupfigcounter}{\value{figure}}
\begin{frame}
    \frametitle{\secname}
    \framesubtitle{\subsecname}
...
\setcounter{figure}{\value{backupfigcounter}}
\begin{frame}
   \frametitle{\secname}
   \framesubtitle{\subsecname}
...
\begin{frame}
    ...
    \begin{minipage}{.4\textwidth}
        ...
        \onslide<2->{
            \begin{enumerate}
                \item generate \textbf{5G graphs}
                \only<3->{\item DQN maximizes revenue under \textbf{federation} and dynamic pricing}
                \only<4->{\item OKpi minimizes \textbf{VNE} cost meeting: latency, reliability, and availability constraints}
                \only<5->{\item TES \& online training \textbf{scaling} reduces the E2E latency violations}
            \end{enumerate}
        }
    \end{minipage}
    \begin{minipage}{.55\textwidth}
        \begin{figure}
            \centering
            \only<1>{\includegraphics[width=\textwidth]{img/big-picture-start.pdf}}%
            \only<2>{\includegraphics[width=\textwidth]{img/big-picture-infra-gen-highlight-ar.pdf}}%
            \only<3>{\includegraphics[width=\textwidth]{img/big-picture-federation.pdf}}%
            \only<4>{\includegraphics[width=\textwidth]{img/big-picture-vne.pdf}}%
            \only<5>{\includegraphics[width=\textwidth]{img/big-picture-scaling.pdf}}%
            \caption{%
                \only<1>{Service orchestration.}%
                \only<2>{Service orchestration --  infrastructure generation.}%
                \only<3>{Service orchestration --  federation.}%
                \only<4>{Service orchestration -- VNE.}%
                \only<5>{Service orchestration -- scaling.}%
            }
        \end{figure}
    \end{minipage}
\end{frame}
        ...
        \begin{itemize}
            \item Conferences:
                \begin{enumerate}
                    ...
                    \item EuCNC~\cite{okpi}
                \end{enumerate}
            \item Journals:
                \begin{enumerate}
                    \item IEEE TB~\cite{modelling-bs}
                    \item IEEE TNSM~\cite{modelling-bs}
...
\begin{frame}
    ...
    \begin{figure}
        \centering
        \includegraphics[width=.3\textwidth]{qr.eps}
    \end{figure}
...
\begin{frame}
    \frametitle{\secname}
    \framesubtitle{Increase of local and federated resources}
...
\begin{frame}
    \frametitle{\secname}
    \framesubtitle{V2N scaling -- forecasting parameters}
    ...
    \small
        ...
        \label{tab:ep}
        %\vspace{-2mm}
        \begin{tabular}{|p{0.3\columnwidth}|p{0.35\columnwidth}|p{0.21\columnwidth}|} \hline
         \textbf{Parameter} & \textbf{Forecasting techniques} &\textbf{Value}  \\ \hline
             Level factor ($\alpha$) & DES, TES & 0.5, 0.5\\ \hline
             Trend factor  ($\beta$)  & DES, TES & 0.001, 0.001\\ \hline
             Seasonality factor ($\gamma$) & TES & 0.001 (3 days)\\  \hline
             Hidden layers & TCN, LSTM, GRU, TCNLSTM & 2,2, 1, 4  \\ \hline
             Neurons in hidden layer & TCN, LSTM, GRU, TCNLSTM & 100  \\ \hline
             Epochs   & TCN, LSTM, GRU, TCNLSTM & 100  \\ \hline
            \multirow{2}{*}{history Window size ($h$)} & TCN, LSTM, TCNLSTM & 60~min. \\ \cline{2-3}
            & GRU & 120~min. \\ \hline
             Batch size  & TCN, LSTM, GRU & 5 \\ \hline
             Temporal memory & \multirow{2}{*}{HTM} & 32x2048\\ \cline{1-1}\cline{3-3}
             Encoder representation & & 1024 bit str\\ \hline
             %Dataset split (Train:Test) & STM & (80:20) \\ \hline
        \end{tabular}
        %\vspace{-3mm}
    \end{table}
